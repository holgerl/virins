\section{Design av løsningen}
	...
	\subsection{Design av LED-headset}
		...
		\begin{figure}[h]
		\centering
		\includegraphics[width=\textwidth]{graphics/circuit.jpg}
	...
	\subsection{Design av posisjonsfinneren og 3D demo}
		    ...
		    Hovedmotivasjonen er at rammeverket skal være utvidtbart slik at det enkelt kan lages andre typer HeadTrackerobjekter,
		    som for eksempel støtter annet utstyr.
...
\section{Implementasjon av løsningen}
	...
	\subsection{Konstruksjon av LED-headset}
		...
		Målet var å finne en løsning som ikke er i veien for brukeren. Optimalt sett såtrenger ikke brukeren ta på seg noe ekstra utstyr. For å kunne tracke brukeren trengs det minimum to lyskilder og 1 kamera, eller to kamera og en lyskilde. Vi bestemte oss for å bruke et trådløst headset, da dette ikke er spesielt tungvindt å ha på. Dette har også et innebygd batteri som vi kan koble lyskilder på. For å kunne skille rotasjon fra avstand trengs det en tredje diode. Denne kan festes på mikrofonen. Se figur \ref{fig:headset}.
	...
	\subsection{Implementasjon av programvaren}
		...
		\subsubsection{Posisjonsfinneren og 3D-viseren (Vegar)}
			...
			Posisjonsfinner ble implementert som i figur \ref{fig:vegar_class_diagram}.
			Noen detaljer er utelatt, for eksempel benytter wiimote-klassene seg av biblioteket 
			wiimote-simple for oppkobling og datauthenting fra wiimote. I tillegg er det lagd noen trivielle klasser som kun brukes til å holde data,
			evt brukes til testing.
			
			Implementasjon av matematikken som beskrevet i forstudien ble gjort i størst mulig grad likt formelverket i forstudien.
		...
		\subsubsection{Bibliotek brukt i implementasjonen}
			...
			Wiimote-simple brukes for å lese ut dataene fra wiimote, som er på et proprietært format.
			
			Wiimotesene kommuniserer gjennom Bluetooth-nettverk. For å snakke med disse bruker vi et Java-bibliotek kalt BlueCove. Dette tilbyr et grensesnitt for å sende og ta imot meldinger gjennom Bluetooth.
		
		\subsubsection{Kjørbar distribusjon (Vegar)}
		
			En kjørbar jar(java archive)-fil lages og pakkes med de nødvendige biblioteker, samt en konfigurasjonsfil headtracker.properties for å konfigurere parametre som påvirker trackingen. Dette skal gjøre det enkelt å få kjørt en demonstrasjon av produktet, uten å ha kjennskap til biblioteker og kompilering.

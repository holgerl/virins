...
public class TestLevel extends Level {
    ...
    public void wallHole() {
        ...
        rectangles.add(new Rectangle3D(new Vector3D(0, -3.0 / 8.0, game.roomDepth), normal, orientation, 1.0, width));
        rectangles.add(new Rectangle3D(new Vector3D(0, 3.0 / 8.0, game.roomDepth), normal, orientation, 1.0, width));
    ...
    void ball() {
        spheres.add(new Sphere(0.1, new Vector3D((Math.random() - .5f) * .9, (Math.random() - .5)*.9, game.roomDepth)));
    }
    void circle() {
        circles.add(new Circle(new Vector3D((Math.random() - .5f)*.9, (Math.random() - .5f)*.9, game.roomDepth), new Vector3D(0.0,0.0,1.0), 0.1));
    }
    public void frame() {
        if((frame + 50L) % 500 == 0) {
            circle();

\section{Design av l\o  sningen}
	...
	\subsection{Design av LED-headset (Jon)}
		...
		Planen for headsettet er at det skal inneholde tre dioder plassert s\aa \space langt unna hverandre som mulig slik at det blir enklest mulig \aa \space skille lyspunktene fra hverandre. Vi vil ha en l\o sning som er grei \aa \space ha p\aa \space hodet, som er portabel og lett. Vi \o nsker \aa \space koble oss direkte til batteriet for \aa \space f\aa \space str\o m til diodene. Her trengs det en bryter for \aa \space ikke kortslutte batteriet. Diodene kobles s\aa \space i parallell over batteriet i serie med hver sin motstand for \aa \space begrense str\o mmen.
		...
		Vi \o nsker at to av diodene st\aa r vannrett p\aa \space hverandre, med en tredje diode over eller under. Vi \o nsker ogs\aa \space \aa \space kunne bytte mellom og ha en og tre dioder p\aa \space om gangen for \aa \space kunne sammenligne ulike tiln\ae rminger til problemet. Ved bare en diode p\aa \space m\aa \space den v\ae re synlig fra st\o rst mulig vinkel. For at diodene skal v\ae re synlige fra st\o rst mulig vinkel uten at hodet til brukeren kommer i veien er det \o nskelig at diodene monteres langt fram p\aa \space headsettet, og at den tredje dioden monteres ved mikrofonen p\aa \space headsettet. 
		...
		\begin{enumerate}
			\item For hvert punkt {\it point}:
				\begin{enumerate}
					...
					\item Om dette blir funnet, s\aa \space er {\it point} = siste av de etterf\o lgende bildepunktene
				\end{enumerate}
		\end{enumerate}
...
\section{Implementasjon av l\o sningen}
	...
	\subsection{Implementasjon av programvaren}
		...
		Progamvaren består av punktfinneren, posisjonsfinneren og 3D-viseren. De er alle skrevet i Java, men 3D-viseren bruker et bibliotek implementert i C for selve visning av 3D-grafikken. Figur \ref{fig:class_diagram} viser et klassediagram av programvaredelen av systemet.
		
		TODO: Beskriv klassediagrammet
		
		TODO: Beskriv biblioteker vi har brukt (DirectShow, JOGL, Bluetooth-shit, osv)
		
		TODO: Legg til tekst om JAR-filen vi har laget, og eventuelle andre filer som må være med en distribusjon
		
		\begin{figure}[h]
		\centering
		%\includegraphics[width=\textwidth]{graphics/class_diagram.png}
		\caption{Klassediagram for programvaredelen av systemet}
